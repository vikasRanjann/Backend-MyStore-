const express = require('express');
const Admin = require('../models/admin')
const crypto = require('crypto-js')
const utils = require('../utils')
const jwt = require('jsonwebtoken')
const config = require('../config')    

const router = express.Router();


//create admin/signup
router.post('/signup',(request,response)=>{

     const {firstName, lastName,password,email, phone} = request.body;

     const admin = new Admin()
     admin.firstName = firstName
     admin.lastName = lastName
     admin.password = crypto.MD5(password)
     admin.email = email
     admin.phone = phone


     admin.save((error,admin)=>{
          response.send(utils.createResult(error,admin));
     })
     

})
//login/admin/signin
router.post('/signin',(request,response)=>{

     const {email,password} = request.body

     const encypt = '' + crypto.MD5(password)
 
     Admin
     .findOne({email: email, password:encypt},{__v:0,password:0,deleted:0,createdTimestamp:0})
     .exec((error,admin)=>{
          if(error) 
          {
               response.send(utils.createResult(error,null));

          }
          else if(!admin)
          {
               response.send(utils.createResult('Invalid user or password',null));
          }
          else
          {
               const payload = {
                    id:admin._id,
                    name:admin.firstName + ' ' + admin.lastName

               }
               const token = jwt.sign(payload,config.secret)
               response.send(utils.createResult(error,{

                    name:admin.firstName + ' ' + admin.lastName,
                    email:admin.email,
                    phone:admin.phone,
                    token:token
                    
               }));


          }
     })
     

})

//all admin data get
router.get('/',(request,response)=>{

     Admin
     .find({deleted:false})
     .exec((error,admin)=>{
          response.send(utils.createResult(error,admin));
     })


})

router.delete('/:id',(request,response)=>{

     const{id} = request.params
     
     Admin
     .findOne({_id:id})
     .exec((error,Admin)=>{

          if(error) 
          {
               response.send(utils.createResult(error,null));

          }
          else if(!Admin)
          {
               response.send(utils.createResult('Admin not found',null));
          }
          else
     {
          Admin.deleted = true;
         
          Admin.save((error,Admin)=>{
               response.send(utils.createResult(error,Admin));
          }) 
     }    
     })



})

module.exports = router